...
export async function bootstrapWorkspaceForUser(session: AuthenticatedUser, input: {
  ...
  for (const department of departments) {
    ...
    await ensureGatewaySession(getCanonicalSessionKey(openclawAgentId), openclawAgentId, config.name)
  }

  await importLegacyMissionControlTasks(workspaceId, projectId)

  const coreAgentRow = (await listWorkspaceAgentRows(workspaceId)).find((agent) => agent.department === 'core')
  ...
  const agentByDepartment = new Map((await listWorkspaceAgentRows(workspaceId)).map((agent) => [agent.department, agent]))
